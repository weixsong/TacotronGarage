...
def conv1d_banks(inputs, K=16, is_training=True, scope="conv1d_banks", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        output = bn(output, is_training=is_training, activation_fn=tf.nn.relu)

    return output  # (N, T, Hp.embed_size//2*K)
...
def highwaynet(inputs, num_units=None, scope="highwaynet", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        y = tf.layers.dense(inputs, units=num_units, activation=tf.nn.relu, name="dense1")
        gate = tf.layers.dense(inputs, units=num_units, activation=tf.nn.sigmoid,
                               bias_initializer=tf.constant_initializer(-1.0), name="dense2")
        outputs = y * gate + inputs * (1. - gate)

    return outputs


def attention_rnn(inputs, memory, num_units=None, scope="attention_rnn", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        outputs, state = tf.nn.dynamic_rnn(cell_with_attention, inputs, dtype=tf.float32)  #( N, T', num_units)
