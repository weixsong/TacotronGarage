...
def encoder(inputs, is_training=True, scope="encoder", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse): 
        ...
        for i in range(hp.num_highwaynet_blocks):
            enc = highwaynet(enc, num_units=hp.embed_size//2, scope='highway_net_{}'.format(i))  # (N, T_x, E/2)
...
def decoder(inputs, memory, is_training=True, scope="decoder", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Attention RNN
        dec, state = attention_rnn(inputs, memory, num_units=hp.embed_size)  # (N, T_y/r, E)
...
def postnet(inputs, is_training=True, scope="postnet", reuse=None):
